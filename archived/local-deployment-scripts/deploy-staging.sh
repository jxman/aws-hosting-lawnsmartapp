...
# Staging Deployment Script for Terraform S3 + CloudFront Infrastructure
# Usage: ./deploy-staging.sh [plan|apply|destroy]
...
terraform_plan() {
    ...
    if [ $? -eq 0 ]; then
        ...
        echo ""
        print_status "Staging environment plan ready for review."
    else
...
terraform_apply() {
    print_status "Applying Terraform configuration to staging..."
    ...
    # Confirmation for staging environment
    echo ""
    print_warning "🧪 Applying changes to STAGING environment"
    print_status "This environment is used for pre-production testing."
...
terraform_destroy() {
    print_warning "⚠️  You are about to destroy the STAGING environment."
    echo ""
    ...
    print_status "Destroying staging resources..."
...
main() {
    clear
    echo "====================================="
    echo "🧪 Terraform Staging Deployment"
    echo "====================================="
    echo ""
